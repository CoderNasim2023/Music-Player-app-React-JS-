...
function App() {
  ...
  const fetchMusicData = async (keyword, offset = 0, limit = 10) => {
    setTracks([]);
    window.scrollTo(0, 0);
    setIsLoading(true);
    try {
      const response = await fetch(
        `https://v1.nocodeapi.com/itsnasim254/spotify/OCFbQqjyHISAPWSo/search?q=${keyword}&type=track&offset=${resultOffset}`,
        {
          headers: {
